...
for (const width of [1280, 375]) {
  test(`creates nested elements, edits cardinality, moves a field, and reopens at ${width}`, async ({ page }) => {
    ...
    expect(
      await page.locator('app-container-editor').evaluate((node) => node.scrollWidth <= node.clientWidth + 1),
    ).toBe(true);
    await page.screenshot({ path: `/tmp/ced-nested-editor-${width}.png`, fullPage: true });
    await page.evaluate((artifact) => {
