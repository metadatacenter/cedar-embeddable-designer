...
test('field and element property choices update only their parent context and cancel preserves it', async ({
  page,
}) => {
  ...
  expect(await picker.evaluate((element: any) => ({ types: element.termTypes, max: element.maximumTerms }))).toEqual({
    types: ['property'],
    max: 1,
  });
  const emit = async (constraints: object[]) =>
    picker.evaluate(
      (element, constraints) =>
...
test('real CETP selects a property for field metadata', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const d = document.querySelector('cedar-embeddable-designer') as any;
    d.artifact = structuredClone(d.currentTemplate);
  });
  ...
  await page.route('**/properties/versions?*', (route) =>
    route.fulfill({ json: [{ id: 'ro-v1', propertiesAvailable: true }] }),
  );
