 ...
 */
function derivedTitle(name: string, kind: 'template' | 'element' | 'field'): string {
  return `${name} ${kind} schema`;
}

function derivedDescription(name: string, kind: 'template' | 'element' | 'field'): string {
...
function artifactMetadataOf(field: AbstractSchemaArtifact): ArtifactMetadata {
  return {
    title: field.title,
...
function applyArtifactMetadata(field: AbstractSchemaArtifact, metadata: ArtifactMetadata): void {
